...
from ayon_animate import api as animate, ANIMATE_ADDON_ROOT
...
class CollectExtensionVersion(pyblish.api.ContextPlugin):
    """ Pulls and compares version of installed extension.
        ...
        `repos/avalon-core/avalon/animate`
    """
    ...
    def process(self, context):
        installed_version = animate.stub().get_extension_version()

        if not installed_version:
            raise PublishError("Unknown version, probably old extension")

        manifest_url = os.path.join(
            ANIMATE_ADDON_ROOT, "api", "extensions", "CSXS", "manifest.xml"
        )
